...
async def upload_structured_ui(file: UploadFile = File(...)):
    """UI 호환 정형 데이터 업로드"""
    return await upload_structured_data(file)

@app.post("/api/v1/data-aug/upload-knowledge")
async def upload_knowledge_ui(files: List[UploadFile] = File(...)):
    """UI 호환 지식 파일 업로드"""
...
async def get_status_ui(task_id: str):
    """UI 호환 상태 조회"""
...
async def download_ui(task_id: str):
    """UI 호환 다운로드"""
