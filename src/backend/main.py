...
@app.post("/api/v1/data-aug/start-augmentation")
async def start_augmentation_simple(
    structured_file_id: str,
    knowledge_batch_id: Optional[str] = None,
    background_tasks: BackgroundTasks = None
):
    """단순화된 데이터 증강 시작 (UI 호환용)"""
    try:
        # 파일 경로 구성
        upload_dir = Path(tempfile.gettempdir()) / "augmentation_uploads"
        ...
        logger.error(f"UI 호환 증강 시작 실패: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/api/v1/data-aug/status/{task_id}")
async def get_augmentation_status_legacy(task_id: str):
    """증강 상태 조회 (UI 호환용)"""
...
async def download_augmented_data_legacy(task_id: str):
    """증강된 데이터 다운로드 (UI 호환용)"""
    # 증강된 데이터 파일 다운로드
